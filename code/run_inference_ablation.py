#!/usr/bin/env python3
"""
Ablation inference for SARDInn.
Identical to run_inference.py but checkpoint/results_dir
    ...
    with torch.no_grad():
        for z in range(z_start, z_start + N_SLICES):
            ...
            for t_idx in range(N_tar):
                ...
                pred    = model.irr(M, q_star_t, q_in_t)
...
def main():
    ...
    print(f"Strategy: DWFE once per slice, IRR loops over target directions")
    ...
    print(f"\n{'='*55}")
    print(f"OVERALL — {args.variant}, scale r={SCALE_R}, {SHELL}")
    print(f"{'='*55}")
